...
export function insertAnnotation(input: CreateAnnotationInput): Annotation {
  ...
  getDb()
    .prepare(
      `INSERT INTO annotations (id, sld_id, page_number, author_type, shape_type, path_data, color, comment_text, created_at)
       VALUES (@id, @sld_id, @page_number, @author_type, @shape_type, @path_data, @color, @comment_text, @created_at)`
    )
    .run(row)
