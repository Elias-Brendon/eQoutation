...
import type { DotMatrixPhase } from '@renderer/components/ui/dotm-square-2-utils/dotmatrix-core'
...
export function useCyclePhase({ active, cycleMsBase, speed = 1 }: UseCyclePhaseOptions): number {
  ...
  useEffect(() => {
    if (!active) {
      // eslint-disable-next-line react-hooks/set-state-in-effect -- resets phase when this hook's `active` flag flips off, not a render-cascade risk
      setPhase(0)
      return
    }
...
export function useSteppedCycle({
  ...
  useEffect(() => {
    if (!active) {
      ...
      // eslint-disable-next-line react-hooks/set-state-in-effect -- resets step when this hook's `active` flag flips off, not a render-cascade risk
      setStep(idleStep)
      return
    }
