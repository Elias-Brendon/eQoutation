/* eslint-disable react-refresh/only-export-components -- vendored dot-matrix engine: intentionally exports many component-factory helpers, resolver functions, and types alongside components from one module */
'use client'

import type { CSSProperties } from 'react'

import '@renderer/components/ui/dotm-square-2-utils/dotmatrix-loader.css'
import { useMemo } from 'react'
import {
  useDotMatrixPhases,
  usePrefersReducedMotion,
  useCyclePhase
} from '@renderer/components/ui/dotm-square-2-utils/dotmatrix-hooks'
...
export function DotMatrix3Base({
  ...
  boxSize,
  minSize
}: DotMatrix3BaseProps): React.JSX.Element {
...
export function createPathWaveComponent(
  displayName: string,
  getPathNorm: NormFn
): (props: PathWaveComponentProps) => React.JSX.Element {
...
export function createDiagonalWave3Component(
  displayName: string,
  direction: DiagonalWave3Direction
): (props: DiagonalWave3ComponentProps) => React.JSX.Element {
  ...
  function DiagonalWave3Component({
    ...
    speed = 1.15,
    ...rest
  }: DiagonalWave3ComponentProps): React.JSX.Element {
    const reducedMotion = usePrefersReducedMotion()
    const {
      phase: matrixPhase,
      onMouseEnter,
...
export function createDotm3x3Component(
  ...
  animationResolver: DotAnimationResolver,
  defaultSpeed = 1.15
): (props: Dotm3x3ComponentProps) => React.JSX.Element {
  function Dotm3x3Component({
    ...
    speed = defaultSpeed,
    ...rest
  }: Dotm3x3ComponentProps): React.JSX.Element {
    const reducedMotion = usePrefersReducedMotion()
    const {
      phase: matrixPhase,
      onMouseEnter,
...
export function createGlyphSpin3Component(
  ...
  defaultSpeed = 1
): (props: GlyphSpin3ComponentProps) => React.JSX.Element {
  function GlyphSpin3Component({
    ...
    hoverAnimated = false,
    ...rest
  }: GlyphSpin3ComponentProps): React.JSX.Element {
    const reducedMotion = usePrefersReducedMotion()
    const {
      phase: matrixPhase,
      onMouseEnter,
