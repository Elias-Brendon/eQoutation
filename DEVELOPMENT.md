...
5. **Verify the publish actually completed — don't trust exit code 0 alone, and don't trust `gh release view` alone either.** `electron-builder --win --publish always` on this project reliably logs its `publishing`/`uploading`/`creating GitHub release` lines **twice** per run — this is not harmless duplicate logging. It actually creates **two separate GitHub Release objects for the same tag**, and splits the three assets (`*-setup.exe`, `*-setup.exe.blockmap`, `latest.yml`) across them essentially at random. `gh release view`/`gh release upload` (which resolve a release **by tag name**) land on whichever one GitHub's tag-lookup returns — often the more complete one, by luck — but the actual app queries `GET /repos/{owner}/{repo}/releases/latest` (`electron-updater`'s `PrivateGitHubProvider`), which GitHub resolves to whichever of the two duplicate objects it considers "latest" by its own internal tie-break — **not necessarily the same one**, and it was empirically the *less* complete one three times running. This produces the confusing failure mode where `gh release view vX.Y.Z` shows all 3 assets correctly present, yet the app still reports "Cannot find latest.yml in the release" or a similarly missing-asset error.

   After every `build:win:publish`, always check for duplicates first:
   ```
   gh api repos/Elias-Brendon/Qoutation/releases --jq '.[] | select(.tag_name=="vX.Y.Z") | {id, assets: [.assets[].name]}'
   ```
   If more than one object comes back for the same tag, pick whichever has (or, combined, could have) all 3 assets, upload anything missing to that one, then **delete the other by its exact numeric ID** (not by tag — `gh release delete` by tag is ambiguous with duplicates present):
   ```
   gh api -X DELETE repos/Elias-Brendon/Qoutation/releases/<bad-id>
   ```
   Then re-verify against the exact endpoint the app uses, not just `gh release view`:
   ```
   gh api repos/Elias-Brendon/Qoutation/releases/latest --jq '{tag_name, assets: [.assets[].name]}'
   ```
   Expect exactly 3 assets and the correct `tag_name`. If `dist/latest.yml` itself is stale (wrong version inside — this has also happened, the local file genuinely wasn't regenerated one run), don't upload it as-is: either rerun `build:win:publish`, or hand-craft it (`version`, `files[0].url`/`sha512`/`size`, top-level `path`/`sha512` matching the installer's actual sha512 in base64 — `node -e "console.log(require('crypto').createHash('sha512').update(require('fs').readFileSync('dist/eqoutation-X.Y.Z-setup.exe')).digest('base64'))"` — and `releaseDate` in ISO 8601). Also note: `gh release upload local/path#asset-name --clobber` (the rename syntax) was observed to silently no-op rather than replace content — if an asset has wrong content and a same-named local file's `--clobber` upload doesn't fix it, `gh release delete-asset vX.Y.Z <name> --yes` followed by a plain upload is the reliable path.
...
**Debugging checklist:**
- **"Check for Updates always fails" with a 404 on `.../releases.atom` in the console:** `electron-updater` fell back to the *public* GitHub provider (which reads the public, unauthenticated Atom feed — 404s on a private repo) instead of the authenticated `PrivateGitHubProvider`. Its provider factory picks based on `process.env.GH_TOKEN`/`GITHUB_TOKEN` being set **in the running process** — not `autoUpdater.requestHeaders`, which does nothing for provider selection. `initAutoUpdater()` (`src/main/updater/autoUpdater.ts`) sets `process.env.GH_TOKEN` from the embedded token specifically to fix this — hit this live on the first real self-update test (v0.1.2 → v0.1.3) before that line existed, with a valid token embedded and a correctly-published release.
- **"Check for Updates always fails" with a 401 in the console:** the embedded token itself is missing, expired, or was revoked — regenerate it on GitHub and rebuild with `build:win:publish` using the new value in `.env`.
- **"No update found even though a newer tag/version exists":** confirm a *GitHub Release* (not just a git tag) exists for that version with `latest.yml` and the installer attached — `npm run release` only tags; only `npm run build:win:publish` actually publishes a Release. `gh release list --repo Elias-Brendon/Qoutation` shows what's actually published, including whether it's still a **draft** — `electron-builder.yml`'s `publish.releaseType: release` is set specifically to avoid this (electron-builder defaults to draft otherwise, which is invisible to unauthenticated fetches/electron-updater — hit this live publishing v0.1.2 before that setting existed).
